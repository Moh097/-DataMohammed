...
taking only the 7 desired countries from the original data set. 7 Wave
```{r}
...
descriptive_summary <- function(data, variables) {
  ...
  for (variable_name in variables) {
    ...
    # Check if the variable contains any missing values (NA)
    if (any(is.na(variable))) {
      cat("Skipping variable with missing values:", variable_name, "\n")
      next
    }
...
# Example Usage
variable_summary <- descriptive_summary(merged_data, variables)
...
# Example Usage
country_variable_summaries <- descriptive_summary(merged_data, variables)
