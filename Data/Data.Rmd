...
model {
  for (i in 1:N) {
    ...
    mu_sign[i] <- alpha_sign + beta_free_choice * free_choice[i] + beta_happiness * happiness[i] + beta_voice * voice_account_score[i] + u_sign[country[i]]
    
    join_boycott[i] ~ dnorm(mu_boycott[i], tau_boycott)
    mu_boycott[i] <- alpha_boycott + beta_life_satis * life_satis[i] + beta_health * health[i] + beta_voice * voice_account_score[i] + u_boycott[country[i]]
    
    attend_demo[i] ~ dnorm(mu_demo[i], tau_demo)
    mu_demo[i] <- alpha_demo + beta_financial_satis * financial_satis[i] + beta_voice * voice_account_score[i] + u_demo[country[i]]
    
    join_strike[i] ~ dnorm(mu_strike[i], tau_strike)
    mu_strike[i] <- alpha_strike + beta_health * health[i] + beta_voice * voice_account_score[i] + u_strike[country[i]]
  }
  
  # Priors for fixed effects
  alpha_sign ~ dnorm(0, 0.001)
  beta_free_choice ~ dnorm(0, 0.001)
  beta_happiness ~ dnorm(0, 0.001)
  
  alpha_boycott ~ dnorm(0, 0.001)
  beta_life_satis ~ dnorm(0, 0.001)
  beta_health ~ dnorm(0, 0.001)

  alpha_demo ~ dnorm(0, 0.001)
  beta_financial_satis ~ dnorm(0, 0.001)
  
  alpha_strike ~ dnorm(0, 0.001)
  
  beta_voice ~ dnorm(0, 0.001)  # Declare beta_voice only once

  # Random effects for countries
  for (j in 1:n_country) {
    u_sign[j] ~ dnorm(0, tau_u_sign)
    u_boycott[j] ~ dnorm(0, tau_u_boycott)
    u_demo[j] ~ dnorm(0, tau_u_demo)
    u_strike[j] ~ dnorm(0, tau_u_strike)
  }
  ...
  tau_u_sign ~ dgamma(0.001, 0.001)
  tau_u_boycott ~ dgamma(0.001, 0.001)
  tau_u_demo ~ dgamma(0.001, 0.001)
  tau_u_strike ~ dgamma(0.001, 0.001)
}
"
...
samples <- coda.samples(jags_model, variable.names = c("alpha_sign", "beta_free_choice", "beta_happiness", "beta_voice", "alpha_boycott", "beta_life_satis", "beta_health", "alpha_demo", "beta_financial_satis", "alpha_strike"), n.iter = 1000)
...
model {
  for (i in 1:N) {
    sign_pet[i] ~ dnorm(mu_sign[i], tau_sign)
    mu_sign[i] <- alpha_sign + beta_free_choice[country[i]] * free_choice[i] + beta_happiness[country[i]] * happiness[i] + beta_voice[country[i]] * voice_account_score[i] + u_sign[country[i]]
    
    join_boycott[i] ~ dnorm(mu_boycott[i], tau_boycott)
    mu_boycott[i] <- alpha_boycott + beta_life_satis[country[i]] * life_satis[i] + beta_health[country[i]] * health[i] + beta_voice[country[i]] * voice_account_score[i] + u_boycott[country[i]]
    
    attend_demo[i] ~ dnorm(mu_demo[i], tau_demo)
    mu_demo[i] <- alpha_demo + beta_financial_satis[country[i]] * financial_satis[i] + beta_voice[country[i]] * voice_account_score[i] + u_demo[country[i]]
    
    join_strike[i] ~ dnorm(mu_strike[i], tau_strike)
    mu_strike[i] <- alpha_strike + beta_health[country[i]] * health[i] + beta_voice[country[i]] * voice_account_score[i] + u_strike[country[i]]
  }
  ...
  for (j in 1:n_country) {
    beta_free_choice[j] ~ dnorm(0, 0.001)
    beta_happiness[j] ~ dnorm(0, 0.001)
    beta_life_satis[j] ~ dnorm(0, 0.001)
    beta_health[j] ~ dnorm(0, 0.001)
    beta_financial_satis[j] ~ dnorm(0, 0.001)
...
# Loop over each unique country in your dataset
for (country_id in unique(final_imputet$country)) {
  
  # Assume you have a way to get the relevant samples subset for this country
  # Replace the next line with your own logic
  country_specific_samples <- subset(samples, country == country_id) # Update this line
  
  # Extract summary statistics for these samples
  country_summary <- summary(country_specific_samples)
  
  # Convert summary into a data.frame for Excel writing
  summary_df <- as.data.frame(as.matrix(country_summary))
  
  # Create a new sheet in the workbook for this country
  sheet_name <- paste("Country", country_id)
  addWorksheet(wb, sheet_name)
  
  # Write the data.frame to this worksheet
  writeData(wb, sheet = sheet_name, x = summary_df)
}

# Save workbook
saveWorkbook(wb, "Country_Specific_Summaries.xlsx", overwrite = TRUE)
