...
merged_renamed <- merged_data_recoded %>%
  mutate(
...
```{r}
# Specify the columns where you want to remove rows with NULL values
columns_to_check <- c("happiness", "health", "sign_pet", "join_boycott", "attend_demo", "join_strike")

# Create a new data frame with rows removed where NULL values are present in the specified columns
merged_renamed_without_NA <- merged_renamed[complete.cases(merged_renamed[, columns_to_check]), ]
...
# List of unique countries
unique_countries <- unique(merged_renamed_without_NA$country)
...
# Loop through each variable and generate the median plot
for (variable_name in variables_to_analyze) {
  median_data_all_countries <- data.frame(
    Country = character(0),  # Initialize an empty character vector
    Median = numeric(0)      # Initialize an empty numeric vector
  )

  # Loop through each unique country
  for (country_code in unique_countries) {
    # Subset data for the current country and variable
    subset_data <- merged_renamed_without_NA[merged_renamed_without_NA$country == country_code, variable_name]
    
    # Check if there are any non-missing values for this country and variable
  ...
  # Check if there is any valid data for the current variable
  if (nrow(median_data_all_countries) > 0) {
    # Create a box plot for median values across all countries
    ...
    # Print the median plot for the current variable
...
# List of unique countries
unique_countries <- unique(merged_imputet_rounded$country)
...
for (ctry in unique_countries) {
  ...
  subset_data <- subset(merged_imputet_rounded, country == ctry)
...
# List of unique countries
unique_countries <- unique(merged_data_recoded$country)
...
for (ctry in unique_countries) {
  # Subset data for each country
  subset_data <- subset(merged_data_recoded, country == ctry)
...
final_imputet$gen_trust <- as.factor(final_imputet$gen_trust)
final_imputet$religiosity <- as.factor(final_imputet$religiosity)
final_imputet$gender <- as.factor(final_imputet$gender)
final_imputet$employment <- as.factor(final_imputet$employment)

# Define the JAGS model
model_string <- "model {
  for (i in 1:N) {
    sign_pet[i] ~ dnorm(mu_sign_pet[i], tau_sign_pet)
    join_boycott[i] ~ dnorm(mu_join_boycott[i], tau_join_boycott)
    
    mu_sign_pet[i] <- alpha_sign_pet[country[i]] + beta_var * var[i] + beta_free_choice * free_choice[i] + gamma_gen_trust[gen_trust[i]] + gamma_religiosity[religiosity[i]]
    mu_join_boycott[i] <- alpha_join_boycott[country[i]] + beta_var * var[i] + beta_free_choice * free_choice[i] + gamma_gen_trust[gen_trust[i]] + gamma_religiosity[religiosity[i]]
  }

  # Priors for country-level intercepts
  for (j in 1:J) {
    alpha_sign_pet[j] ~ dnorm(0, 0.001)
    alpha_join_boycott[j] ~ dnorm(0, 0.001)
  }
  
  # Priors for categorical variables
  for (k in 1:K_gen_trust) {
    gamma_gen_trust[k] ~ dnorm(0, 0.001)
  }
  for (l in 1:K_religiosity) {
    gamma_religiosity[l] ~ dnorm(0, 0.001)
  }

  # Priors for slopes
  beta_var ~ dnorm(0, 0.001)
  beta_free_choice ~ dnorm(0, 0.001)
  
  # Priors for precision parameters
  tau_sign_pet ~ dgamma(0.01, 0.01)
  tau_join_boycott ~ dgamma(0.01, 0.01)
}
"
...
data_jags <- list(
  ...
  J = length(unique(final_imputet$country)),
  K_gen_trust = length(unique(final_imputet$gen_trust)),
  K_religiosity = length(unique(final_imputet$religiosity)),
  country = as.integer(final_imputet$country),
  gen_trust = as.integer(final_imputet$gen_trust),
  religiosity = as.integer(final_imputet$religiosity),
  sign_pet = final_imputet$sign_pet,
  join_boycott = final_imputet$join_boycott,
  free_choice = final_imputet$free_choice,
  var = final_imputet$var
)

# Initialize parameters
params_init <- list(
  alpha_sign_pet = rep(0, length(unique(final_imputet$country))),
  alpha_join_boycott = rep(0, length(unique(final_imputet$country))),
  gamma_gen_trust = rep(0, length(unique(final_imputet$gen_trust))),
  gamma_religiosity = rep(0, length(unique(final_imputet$religiosity))),
  beta_var = 0,
  beta_free_choice = 0,
  tau_sign_pet = 1,
  tau_join_boycott = 1
...
params_to_sample <- c("alpha_sign_pet", "alpha_join_boycott", "gamma_gen_trust", "gamma_religiosity", "beta_var", "beta_free_choice", "tau_sign_pet", "tau_join_boycott")

# Run JAGS model
jags_model <- jags.model(textConnection(model_string), data = data_jags, inits = params_init, n.chains = 3)
update(jags_model, 1000)  # Burn-in

# Generate MCMC samples
samples <- coda.samples(jags_model, variable.names = params_to_sample, n.iter = 5000)
...
# Density plot
densplot(samples, xlim = c(-3, 3))

# Joint Marginal plot for selected parameters
pairs(samples, pars = c("beta_var", "beta_free_choice"))
...
model {
  for (i in 1:N) {
    ...
    mu_sign_pet[i] <- alpha_sign_pet[country[i]] + beta_my_var * my_var[i] + beta_free_choice * free_choice[i] + gamma_gen_trust[gen_trust[i]] + gamma_religiosity[religiosity[i]]
    mu_join_boycott[i] <- alpha_join_boycott[country[i]] + beta_my_var * my_var[i] + beta_free_choice * free_choice[i] + gamma_gen_trust[gen_trust[i]] + gamma_religiosity[religiosity[i]]
  }
...
data_jags <- list(
  ...
  join_boycott = final_imputet$join_boycott,
  free_choice = final_imputet$free_choice,
  my_var = final_imputet$my_var  # Add my_var
)
...
nominal_vars <- c('gen_trust', 'religiosity', 'gender', 'employment', 'country')
final_imputet[nominal_vars] <- lapply(final_imputet[nominal_vars], factor)

# Prepare the data list for JAGS
data_jags <- list(
  N = nrow(final_imputet),
  Y = final_imputet[,c('sign_pet', 'join_boycott', 'attend_demo', 'join_strike')],
  IVs = final_imputet[,c('free_choice', 'life_satis', 'financial_satis', 'happiness', 'health')],
  Ctrls = final_imputet[,c('age', 'gen_trust', 'religiosity', 'gender', 'employment')],
  country = final_imputet$country,
  var = final_imputet$var,
  K = 4,  # number of dependent variables
...
model {
  for(i in 1:N) {
    for(k in 1:K) {
      Y[i,k] ~ dnorm(mu[i,k], prec[k])
      mu[i,k] <- alpha[k] + inprod(beta[k,1:P], IVs[i,1:P]) + u[country[i],k] + inprod(gamma[k,1:5], Ctrls[i,1:5])
    }
  }
  ...
  for(k in 1:K) {
    ...
    for(p in 1:P) {
      beta[k,p] ~ dnorm(0, 0.001)
    }
    for(c in 1:5) {
      gamma[k,c] ~ dnorm(0, 0.001)
    }
  }
...
# Sampling phase
samples <- coda.samples(model, variable.names = c('alpha', 'beta', 'u', 'prec', 'gamma'), n.iter = 5000)
...
library(corrplot)  # Add corrplot package

# Calculate the overall correlation matrix for all countries
overall_corr_matrix <- rcorr(as.matrix(final_dataset[, var_of_interest]))$r

# Visualize the overall correlation matrix
corrplot(overall_corr_matrix, method = "color")
...
# Calculate the overall p-values for all countries
overall_p_values <- rcorr(as.matrix(final_dataset[, var_of_interest]))$P

# Visualize the p-values for the overall correlation matrix
corrplot(overall_p_values, method = "color")
...
significant_corr_matrix <- overall_corr_matrix * (overall_p_values < alpha)

# Create a color palette for the heatmap
color_palette <- colorRampPalette(c("blue", "white", "red"))(n = 100)

# Visualize the significant correlations using a heatmap
corrplot(significant_corr_matrix, method = "color", col = color_palette,
         is.corr = FALSE, addCoef.col = "black", number.cex = 0.7)
